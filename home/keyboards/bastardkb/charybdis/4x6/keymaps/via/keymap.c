...
bool handle_key_event(uint16_t keycode, keyrecord_t *record_ptr, bool physical_key_is_down[MATRIX_ROWS * MATRIX_COLS], uint16_t keys_down, uint16_t index, uint8_t row, uint8_t col, bool key_is_going_down, bool some_other_physical_key_is_down) {
    ...
    bool ctrl_physical_key_is_down = physical_key_is_down[12];
    bool shift_physical_key_is_down = physical_key_is_down[18] || physical_key_is_down[48];
    bool alt_physical_key_is_down = physical_key_is_down[27];
    bool gui_physical_key_is_down = physical_key_is_down[28];
    ...
    // spacebar modifier
    // 
        if (physical_key_is_down[57] && key_is_going_down) {
            // 
            // backspace
            // 
            if (index == 37) {
